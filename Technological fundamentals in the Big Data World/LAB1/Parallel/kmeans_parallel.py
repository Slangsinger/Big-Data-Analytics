...
class kmeans():
    ...
    def _run_kmeans_for_k(args):
        ...
        for _ in range(max_iter):
            # Vectorized distance computation: (n_samples, k)
    ...
    def parallel(self, data, max_iter=200):
        """Parallelized + vectorized k-means for all k."""
        ...
        # Run in parallel across available CPUs
        with Pool(processes=cpu_count()) as pool:
            results = pool.map(self._run_kmeans_for_k, args)
...
if __name__ == "__main__":
    ...
    multiprocessing.freeze_support()  # <-- needed on Windows/macOS
