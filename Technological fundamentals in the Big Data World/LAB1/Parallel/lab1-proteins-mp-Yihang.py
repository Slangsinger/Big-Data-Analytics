...
def run_parallel_elbow(raw_data, k_list, max_iters=300, tol=1e-4, processes=None):
    with mp.get_context("spawn").Pool(processes=processes) as pool:
        results = pool.starmap(
            elbow_worker,
            [(raw_data, k, max_iters, tol) for k in k_list]
        )
...
if __name__ == "__main__":
    ...
    ks = list(range(1, 16))
    inertia_list = run_parallel_elbow(features, ks, processes=min(len(ks), mp.cpu_count()))
    ...
    plt.title("Elbow Plot (WSS vs k) — MP")
    plt.xlabel("k")
    ...
    plt.title("K-means Clusters (Enzyme vs Hydrofob) — MP")
    plt.xlabel("enzyme")
    plt.ylabel("hydrofob")
    plt.legend(loc="best")
    plt.grid(True)
